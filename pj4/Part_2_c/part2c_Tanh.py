# -*- coding: utf-8 -*-
"""
Created on Mon Feb 26 23:40:57 2018
...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 26 13:32:37 2018
...
##Use Tanh activity function for nn
...
for neuron_no in range(1, max_neurons+1):
    ...
    Tanh_nn = MLPRegressor(hidden_layer_sizes = (neuron_no,), \
                            activation = 'tanh', verbose = False)
    train_rmse_list = []
    test_rmse_list = []
    for train_index, test_index in kf.split(feature_ohc_list):
        Tanh_nn.fit(feature_ohc_array[train_index], outputs[train_index])
        train_rmse = rmse(outputs[train_index],Tanh_nn.predict(feature_ohc_array[train_index]))
        test_rmse = rmse(outputs[test_index],Tanh_nn.predict(feature_ohc_array[test_index]))
...
plt.xlabel('Number of hidden neurons (Activity Function = Tanh)')
...
Tanh_nn = MLPRegressor(hidden_layer_sizes = (best_neuron_no[0],), \
                            activation = 'tanh', verbose = False)
train_rmse_list = []
test_rmse_list = []
for train_index, test_index in kf.split(feature_ohc_list):
...
fitted_values = Tanh_nn.predict(feature_ohc_array)
true_values = outputs
...
plt.title('Fitted Values VS. True Values for Neural Network (Tanh)')
plt.show()
fig.savefig('Tanh_fitted.png')
...
plt.xlabel('True values')     
plt.ylabel('Residues')
plt.title('Residues VS. True Values for Neural Network (Tanh)')
plt.show()
fig.savefig('Tanh_res.png')
