...
##Use Relu activity function for nn
...
for neuron_no in range(1, max_neurons+1):
    ...
    for train_index, test_index in kf.split(feature_ohc_list):
        relu_nn.fit(feature_ohc_array[train_index], outputs[train_index])
        train_rmse = rmse(outputs[train_index],relu_nn.predict(feature_ohc_array[train_index]))
        test_rmse = rmse(outputs[test_index],relu_nn.predict(feature_ohc_array[test_index]))
...
fitted_values = relu_nn.predict(feature_ohc_array)
true_values = outputs
